...
TEST_CASE("PenaltyManager cost_evaluator returns current weights", "[search][penalty_manager]") {
    PenaltyManager pm;
...
TEST_CASE("Penalties decrease when all solutions are feasible", "[search][penalty_manager]") {
...
TEST_CASE("Penalties increase when no solutions are feasible", "[search][penalty_manager]") {
    PenaltyManager pm(0.5, 0.1, 10);
...
TEST_CASE("Penalties unchanged when feasibility matches target", "[search][penalty_manager]") {
    PenaltyManager pm(0.5, 0.1, 10);
...
TEST_CASE("No update before interval is reached", "[search][penalty_manager]") {
...
TEST_CASE("Multiple update cycles accumulate", "[search][penalty_manager]") {
    PenaltyManager pm(0.5, 0.1, 10);
...
TEST_CASE("Penalties are clamped to minimum", "[search][penalty_manager]") {
...
TEST_CASE("Penalties are clamped to maximum", "[search][penalty_manager]") {
...
TEST_CASE("feasible_fraction tracks current window", "[search][penalty_manager]") {
    PenaltyManager pm(0.5, 0.1, 10);
...
TEST_CASE("Window resets after update", "[search][penalty_manager]") {
