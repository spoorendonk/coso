...
TEST_CASE("SimulatedAnnealing — probabilistically accepts worsening", "[acceptance]") {
...
TEST_CASE("SimulatedAnnealing — low temperature rejects worsening", "[acceptance]") {
...
TEST_CASE("SimulatedAnnealing — temperature decays", "[acceptance]") {
    ...
    CHECK_THAT(sa.temperature(), Catch::Matchers::WithinAbs(100.0, 1e-9));

    sa.iteration(100);
    CHECK_THAT(sa.temperature(), Catch::Matchers::WithinAbs(50.0, 1e-9));

    sa.iteration(100);
    CHECK_THAT(sa.temperature(), Catch::Matchers::WithinAbs(25.0, 1e-9));
}

TEST_CASE("SimulatedAnnealing — zero temperature rejects all worsening", "[acceptance]") {
    SimulatedAnnealing sa(100.0, 0.0);  // alpha=0 => temp becomes 0 after 1 iter
    sa.init(100);
    sa.iteration(100);

    CHECK_THAT(sa.temperature(), Catch::Matchers::WithinAbs(0.0, 1e-15));
...
TEST_CASE("RecordToRecord — threshold decays linearly", "[acceptance]") {
    ...
    CHECK_THAT(rtr.threshold(), Catch::Matchers::WithinAbs(10.0, 1e-9));

    rtr.iteration(100);
    CHECK_THAT(rtr.threshold(), Catch::Matchers::WithinAbs(8.0, 1e-9));

    rtr.iteration(100);
    CHECK_THAT(rtr.threshold(), Catch::Matchers::WithinAbs(6.0, 1e-9));
    ...
    CHECK_THAT(rtr.threshold(), Catch::Matchers::WithinAbs(0.0, 1e-9));
}
