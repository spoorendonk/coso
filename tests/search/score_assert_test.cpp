...
TEST_CASE("assert_route_consistency: empty route passes", "[score_assert]") {
...
TEST_CASE("assert_route_consistency: single-client route passes", "[score_assert]") {
    auto data = make_test_instance();
    Route route(data, 0);
    route.insert(0, 0);
...
TEST_CASE("assert_route_consistency: multi-client route passes", "[score_assert]") {
    auto data = make_test_instance();
    Route route(data, 0);
...
TEST_CASE("assert_route_consistency: after insert/remove", "[score_assert]") {
    auto data = make_test_instance();
    Route route(data, 0);
...
TEST_CASE("assert_solution_consistency: empty solution passes", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
    CostEvaluator eval(100);

    REQUIRE_NOTHROW(debug::assert_solution_consistency(sol, eval, data));
}

TEST_CASE("assert_solution_consistency: fully assigned solution passes", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
    CostEvaluator eval(100);
...
TEST_CASE("assert_solution_consistency: partially assigned solution passes", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
    CostEvaluator eval(100);
...
TEST_CASE("assert_solution_consistency: after modifications", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
...
TEST_CASE("assert_cost_delta: validates insert delta evaluation", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
    CostEvaluator eval(100);
    ...
    REQUIRE_NOTHROW(debug::assert_cost_delta(predicted, actual, "insert client 2 at pos 1"));
}

TEST_CASE("assert_cost_delta: validates remove delta evaluation", "[score_assert]") {
    auto data = make_test_instance();
    Solution sol(data);
    CostEvaluator eval(100);
    ...
    REQUIRE_NOTHROW(debug::assert_cost_delta(predicted, actual, "remove client at pos 1"));
}
...
TEST_CASE("assert_solution_consistency: overloaded route passes", "[score_assert]") {
    ProblemData::Builder b;
    b.add_depot({0.0, 0.0});
