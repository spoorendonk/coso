...
TEST_CASE("replan -- pinned clients stay in their route and position",
          "[replan][warm_start]")
{
    ...
    auto route0_before = std::vector<int>(sol.route(0).clients().begin(),
                                          sol.route(0).clients().end());
    auto route1_before = std::vector<int>(sol.route(1).clients().begin(),
                                          sol.route(1).clients().end());
    ...
    auto route0_after = std::vector<int>(sol.route(0).clients().begin(),
                                         sol.route(0).clients().end());
    auto route1_after = std::vector<int>(sol.route(1).clients().begin(),
                                         sol.route(1).clients().end());
...
TEST_CASE("replan -- new clients are inserted into routes",
          "[replan][warm_start]")
{
...
TEST_CASE("replan -- removed clients are no longer in routes",
          "[replan][warm_start]")
{
...
TEST_CASE("replan -- pinned and removed overlap throws",
          "[replan][warm_start]")
{
    auto data = make_replan_cvrp();
    CostEvaluator eval;
...
TEST_CASE("replan -- new client already assigned throws",
          "[replan][warm_start]")
{
    auto data = make_replan_cvrp();
    CostEvaluator eval;
...
TEST_CASE("replan -- pinned client out of range throws",
          "[replan][warm_start]")
{
    auto data = make_replan_cvrp();
    CostEvaluator eval;
...
TEST_CASE("cheapest_insert -- inserts client at best position",
          "[replan][warm_start]")
{
    ProblemData::Builder b;
    b.add_depot({0.0, 0.0});
...
TEST_CASE("local_search_with_pins -- pins are respected",
          "[replan][warm_start]")
{
    auto data = make_replan_cvrp();
    CostEvaluator eval;
...
TEST_CASE("replan -- combined: pin + insert + remove",
          "[replan][warm_start]")
{
    ProblemData::Builder b;
    b.add_depot({0.0, 0.0});
    ...
    for (int p = 0; p < sol.route(0).size(); ++p) {
        if (sol.route(0).client(p) == 0) client0_in_route0 = true;
        if (sol.route(0).client(p) == 1) client1_in_route0 = true;
    }
