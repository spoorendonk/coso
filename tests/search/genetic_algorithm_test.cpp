...
TEST_CASE("GeneticAlgorithm: constructs without error", "[search][ga]") {
...
TEST_CASE("GeneticAlgorithm: run returns a solution with all clients assigned", "[search][ga]") {
    auto data = make_ga_instance();
    CostEvaluator eval;
...
TEST_CASE("GeneticAlgorithm: run with time limit", "[search][ga]") {
    auto data = make_ga_instance();
    CostEvaluator eval;
...
TEST_CASE("GeneticAlgorithm: more iterations yield equal or better solutions", "[search][ga]") {
...
TEST_CASE("GeneticAlgorithm: returns feasible solution on easy instance", "[search][ga]") {
    auto data = make_ga_instance();
    CostEvaluator eval;
...
TEST_CASE("GeneticAlgorithm: same seed produces same result", "[search][ga]") {
    auto data = make_ga_instance();
    CostEvaluator eval;
...
TEST_CASE("GeneticAlgorithm: different seeds can produce different results", "[search][ga]") {
    auto data = make_larger_instance();
    CostEvaluator eval;
...
TEST_CASE("GeneticAlgorithm: custom population sizes work", "[search][ga]") {
    auto data = make_ga_instance();
    CostEvaluator eval;
