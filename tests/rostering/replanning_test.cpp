#include "rostering/replanning.h"
...
RosteringData make_replan_instance() {
    RosteringData data;
    ...
    data.employees = {
        ...
        {.name = "Dave", .skills = {"nurse"}, .max_consecutive_days = 5},
    };
...
TEST_CASE("LockedCells: horizon_start locks early days", "[rostering][replanning]") {
...
TEST_CASE("LockedCells: explicit locked assignments", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: locked shifts remain unchanged", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: new unavailabilities are respected", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: partial replanning (only future days change)", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: simple rostering instance end-to-end", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: demand is still met after replanning", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
...
TEST_CASE("replan: new preferences influence solution", "[rostering][replanning]") {
    auto data = make_replan_instance();
...
TEST_CASE("replan: empty config produces valid solution", "[rostering][replanning]") {
    auto data = make_replan_instance();
    RosteringCostEvaluator evaluator(data);
    auto sol = construct_greedy(data, evaluator);
