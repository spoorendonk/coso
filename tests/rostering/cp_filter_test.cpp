...
TEST_CASE("CPFilter: forbidden sequence prunes completing shift", "[rostering][cp_filter]") {
    auto data = make_instance();
...
TEST_CASE("CPFilter: forbidden sequence prunes first element too", "[rostering][cp_filter]") {
    auto data = make_instance();
...
TEST_CASE("CPFilter: demand limit prunes when at max", "[rostering][cp_filter]") {
    auto data = make_instance();
    // Night max = 1 per day.
...
TEST_CASE("CPFilter: unavailability prunes all shifts", "[rostering][cp_filter]") {
    auto data = make_instance();
...
TEST_CASE("CPFilter: unassign moves are never filtered", "[rostering][cp_filter]") {
    auto data = make_instance();
    data.unavailabilities.insert(RosteringData::unavail_key(0, 0));
...
TEST_CASE("CPFilter: multiple rules interact correctly", "[rostering][cp_filter]") {
    ...
    data.forbidden_sequences = {{1, 0}};  // Night -> Day
    data.unavailabilities.insert(RosteringData::unavail_key(2, 3));
