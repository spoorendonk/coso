...
TEST_CASE("AutomatonConstraint: max consecutive resets after break", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
    auto dfa = build_max_consecutive_dfa(NIGHT, 2, data.num_shift_types());
    AutomatonConstraint c(std::move(dfa), 10000);
...
TEST_CASE("AutomatonConstraint: forbidden Night->Day pattern", "[rostering][automaton]") {
    auto data = make_instance();
    int const DAY = 0;
    int const NIGHT = 1;
...
TEST_CASE("AutomatonConstraint: forbidden pattern — Day->Night is OK", "[rostering][automaton]") {
    auto data = make_instance();
    int const DAY = 0;
    int const NIGHT = 1;
    auto dfa = build_forbidden_pattern_dfa({NIGHT, DAY}, data.num_shift_types());
...
TEST_CASE("AutomatonConstraint: forbidden 3-symbol pattern", "[rostering][automaton]") {
    auto data = make_instance();
    int const DAY = 0;
    int const NIGHT = 1;
    int const EVE = 2;
...
TEST_CASE("AutomatonConstraint: delta matches full — max consecutive", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
    auto dfa = build_max_consecutive_dfa(NIGHT, 2, data.num_shift_types());
    AutomatonConstraint c(std::move(dfa), 10000);
    ...
    RosteringMove move{.employee = 0, .day = 2, .old_shift = -1, .new_shift = NIGHT};
...
TEST_CASE("AutomatonConstraint: delta matches full — forbidden pattern", "[rostering][automaton]") {
    ...
    RosteringMove move{.employee = 0, .day = 1, .old_shift = -1, .new_shift = DAY};
    int delta = c.evaluate_delta(data, sched, move);
...
TEST_CASE("AutomatonConstraint: delta — removing violation", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
    auto dfa = build_max_consecutive_dfa(NIGHT, 2, data.num_shift_types());
    AutomatonConstraint c(std::move(dfa), 10000);
    ...
    RosteringMove move{.employee = 0, .day = 1, .old_shift = NIGHT, .new_shift = -1};
    int delta = c.evaluate_delta(data, sched, move);
...
TEST_CASE("AutomatonConstraint: violations across multiple employees", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
    auto dfa = build_max_consecutive_dfa(NIGHT, 2, data.num_shift_types());
    AutomatonConstraint c(std::move(dfa), 10000);
...
TEST_CASE("AutomatonConstraint: works in ConstraintEvaluator", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
...
TEST_CASE("AutomatonConstraint: forbidden pattern including off day", "[rostering][automaton]") {
    auto data = make_instance();
    int const NIGHT = 1;
