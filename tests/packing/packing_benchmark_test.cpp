...
static int local_search(PackingSolution& sol) {
    ...
    while (improved) {
        ...
        if (!merges.empty()) {
            ...
            auto best =
                std::min_element(merges.begin(), merges.end(),
...
static void run_packing_benchmark(std::string const& name, int capacity,
                                  std::vector<int> const& sizes, int optimal_bins,
...
TEST_CASE("Falkenauer U: 10 items, capacity 150", "[benchmark][packing]") {
    ...
    run_packing_benchmark("Falkenauer-U-10", 150, {97, 85, 72, 63, 62, 57, 55, 49, 20, 100}, 5);
}
...
TEST_CASE("Falkenauer U: 20 items, capacity 150", "[benchmark][packing]") {
    ...
    run_packing_benchmark(
        "Falkenauer-U-20", 150,
        {95, 90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 98, 72, 67, 43}, 8, 9);
}
...
TEST_CASE("Falkenauer U: 30 items, capacity 150", "[benchmark][packing]") {
    ...
    run_packing_benchmark("Falkenauer-U-30", 150,
                          {99, 94, 89, 85, 80, 76, 73, 70, 66, 62, 58, 55, 52, 49, 46,
                           43, 40, 37, 34, 31, 28, 25, 22, 97, 91, 83, 78, 68, 60, 48},
                          12, 13);
}
...
TEST_CASE("Falkenauer T: 9 items (3 triplets)", "[benchmark][packing]") {
    ...
    run_packing_benchmark("Falkenauer-T-9", 1000, {490, 450, 480, 300, 320, 280, 200, 220, 240}, 3,
                          4);
}
...
TEST_CASE("Falkenauer T: 15 items (5 triplets)", "[benchmark][packing]") {
    ...
    run_packing_benchmark(
        "Falkenauer-T-15", 1000,
        {485, 460, 470, 440, 490, 310, 330, 290, 350, 270, 200, 210, 235, 205, 240}, 5, 6);
}

TEST_CASE("Falkenauer T: 30 items (10 triplets)", "[benchmark][packing]") {
    // 10 bins optimal.
    run_packing_benchmark(
        "Falkenauer-T-30", 1000,
        {490, 485, 470, 460, 455, 445, 440, 435, 420, 410, 310, 305, 295, 290, 285,
         280, 275, 270, 265, 260, 200, 210, 235, 250, 240, 225, 215, 245, 205, 230},
        10, 11);
}
...
TEST_CASE("Scholl Class 1: 25 items, cap 100", "[benchmark][packing]") {
    ...
    run_packing_benchmark("Scholl-C1-25", 100, {95, 88, 79, 71, 63, 56, 48, 41, 33, 26, 19, 14, 9,
                                                5,  2,  97, 82, 67, 52, 38, 24, 11, 7,  3,  1},
                          10, 11);
}
...
TEST_CASE("Scholl Class 2: medium items, cap 150", "[benchmark][packing]") {
    ...
    run_packing_benchmark(
        "Scholl-C2-20", 150,
        {49, 47, 45, 43, 41, 39, 37, 35, 33, 31, 29, 27, 50, 48, 46, 44, 42, 40, 38, 36}, 5, 6);
}
...
TEST_CASE("Scholl Class 3: large items, cap 100", "[benchmark][packing]") {
    ...
    run_packing_benchmark("Scholl-C3-10", 100, {49, 47, 45, 43, 41, 39, 37, 35, 26, 23}, 5, 5);
}
...
TEST_CASE("Scholl Class 3: 20 items, cap 100", "[benchmark][packing]") {
    ...
    run_packing_benchmark(
        "Scholl-C3-20", 100,
        {50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 31}, 10, 11);
}
