...
TEST_CASE("PiecewiseLinearFunction: requires at least 2 breakpoints",
          "[piecewise_cost]")
{
    CHECK_THROWS_AS(
...
TEST_CASE("PiecewiseLinearFunction: requires strictly increasing x",
          "[piecewise_cost]")
{
    CHECK_THROWS_AS(
        PiecewiseLinearFunction({{0, 0}, {0, 1}}),
        std::invalid_argument);

    CHECK_THROWS_AS(
        PiecewiseLinearFunction({{0, 0}, {5, 5}, {3, 3}}),
        std::invalid_argument);
}
...
TEST_CASE("PiecewiseLinearFunction: simple linear (identity-like)",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction: extrapolation below first breakpoint",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction: extrapolation above last breakpoint",
          "[piecewise_cost]")
{
    PiecewiseLinearFunction f({{0, 0}, {10, 20}});
...
TEST_CASE("PiecewiseLinearFunction: multi-segment interpolation",
          "[piecewise_cost]")
{
    // Three segments:
    //   [0,0] -> [100, 100]   slope = 1
...
TEST_CASE("PiecewiseLinearFunction: negative slope segments",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction: delta same x returns zero",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction: delta on same segment is O(1)",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction::tiered: basic two-tier",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction::tiered: three-tier",
          "[piecewise_cost]")
{
...
{
    CHECK_THROWS_AS(
        PiecewiseLinearFunction::tiered({100}, {1}),
        std::invalid_argument);

    CHECK_THROWS_AS(
        PiecewiseLinearFunction::tiered({100}, {1, 2, 3}),
        std::invalid_argument);
}
...
TEST_CASE("PiecewiseLinearFunction::overtime: single tier",
          "[piecewise_cost]")
{
...
TEST_CASE("PiecewiseLinearFunction::overtime: two tiers",
          "[piecewise_cost]")
{
...
static ProblemData make_piecewise_instance()
{
    ...
    b.add_vehicle_type(2, {
        .capacity = {100},
        .cost = {.fixed_cost = 0, .unit_distance_cost = 1}
    });
          ...
          "[piecewise_cost][cost_evaluator]")
{
    ...
    eval_pw.set_distance_cost_function(
        PiecewiseLinearFunction::tiered({20}, {1, 3}));

    CHECK(eval_pw.route_objective(route) == 80);
    CHECK(eval_pw.has_distance_cost_function());
}

TEST_CASE("CostEvaluator: clear piecewise reverts to linear",
          "[piecewise_cost][cost_evaluator]")
{
    auto data = make_piecewise_instance();
    Route route(data, 0);
    ...
    CostEvaluator eval;
    eval.set_distance_cost_function(
        PiecewiseLinearFunction::tiered({20}, {1, 3}));
...
TEST_CASE("CostEvaluator: tiered pricing scenario end-to-end",
          "[piecewise_cost][cost_evaluator]")
{
