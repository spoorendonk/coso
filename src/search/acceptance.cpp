...
LateAcceptance::LateAcceptance(int list_length) : list_length_(list_length) {}
...
SimulatedAnnealing::SimulatedAnnealing(double initial_temp, double alpha, unsigned int seed)
    : initial_temp_(initial_temp), alpha_(alpha), temp_(initial_temp), rng_(seed) {}

void SimulatedAnnealing::init([[maybe_unused]] int64_t cost) {
    temp_ = initial_temp_;
}

bool SimulatedAnnealing::accept(int64_t candidate_cost, int64_t current_cost) {
...
bool RecordToRecord::accept(int64_t candidate_cost, [[maybe_unused]] int64_t current_cost) {
    if (candidate_cost < best_cost_) {
        best_cost_ = candidate_cost;
    }

    return static_cast<double>(candidate_cost) <= static_cast<double>(best_cost_) + threshold_;
}
...
AcceptanceCriterion::AcceptanceCriterion(LateAcceptance la) : impl_(std::move(la)) {}

AcceptanceCriterion::AcceptanceCriterion(SimulatedAnnealing sa) : impl_(std::move(sa)) {}

AcceptanceCriterion::AcceptanceCriterion(RecordToRecord rtr) : impl_(std::move(rtr)) {}

void AcceptanceCriterion::init(int64_t cost) {
    std::visit([cost](auto& criterion) { criterion.init(cost); }, impl_);
}

bool AcceptanceCriterion::accept(int64_t candidate_cost, int64_t current_cost) {
    return std::visit(
...
void AcceptanceCriterion::iteration(int64_t current_cost) {
    std::visit([current_cost](auto& criterion) { criterion.iteration(current_cost); }, impl_);
}
