...
Population::Population(ProblemData const& data, int max_feasible, int max_infeasible, int num_close)
    : data_(&data),
...
Solution const& Population::best_feasible() const {
    assert(has_feasible());
    auto it =
        std::min_element(feasible_.begin(), feasible_.end(),
                         [](Individual const& a, Individual const& b) { return a.cost < b.cost; });
    return it->sol;
}
...
std::vector<std::pair<int, int>> Population::extract_edges(Solution const& sol) {
...
std::vector<double> Population::biased_fitness(SubPop const& pop) const {
    ...
    if (nc > 0 && n > 1) {
        for (int i = 0; i < n; ++i) {
            ...
            std::partial_sort(dists.begin(), dists.begin() + nc, dists.end());
            double sum = 0.0;
            for (int k = 0; k < nc; ++k) {
                sum += dists[k];
            }
...
Solution const& Population::tournament(std::mt19937& rng) const {
