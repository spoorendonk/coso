...
struct SyncResource {
    ...
    build_lookup(int num_clients, std::vector<SyncGroup> const& groups) {
    ...
    [[nodiscard]] static State init(std::vector<int> const& group_lookup,
                                    int client, int arrival) {
        assert(client >= 0
               && client < static_cast<int>(group_lookup.size()));
        State s;
    ...
    [[nodiscard]] static State merge(State const& left, State const& right) {
        ...
        result.entries.insert(result.entries.end(),
                              right.entries.begin(), right.entries.end());
        return result;
    }
    ...
    [[nodiscard]] static int excess(
            State const& route_a, State const& route_b,
