...
inline std::vector<int> build_request_routes(Solution const& sol, ProblemData const& data,
...
inline std::vector<int> remove_clients_at(Route const& route, std::vector<int> const& positions) {
...
inline int64_t eval_route_cost(ProblemData const& data, CostEvaluator const& eval, int vehicle_type,
...
inline int64_t find_best_pair_insertion(std::vector<int> const& base_clients,
                                        ProblemData const& data, CostEvaluator const& eval,
                                        int vehicle_type, int pickup, int delivery, int& best_p,
                                        int& best_d) {
...
bool RelocatePair::find_best_move(Solution const& sol, CostEvaluator const& eval,
    ...
    for (int r = 0; r < static_cast<int>(requests.size()); ++r) {
        ...
        int64_t cost_a_without = eval_route_cost(data, eval, route_a.vehicle_type(), reduced_a);
        ...
        auto try_route = [&](int rb) {
            ...
            int vt = (rb == ra) ? route_a.vehicle_type() : route_b.vehicle_type();

            int bp, bd;
            int64_t new_cost_b =
                find_best_pair_insertion(base, data, eval, vt, pickup, delivery, bp, bd);
...
void RelocatePair::apply(Solution& sol) const {
    ...
    if (from_route_ == to_route_) {
        ...
        int pos_p = -1, pos_d = -1;
        for (int i = 0; i < route.size(); ++i) {
            if (route.client(i) == pickup_) {
                pos_p = i;
            }
            if (route.client(i) == delivery_) {
                pos_d = i;
            }
        }
        ...
        for (int i = 0; i < src.size(); ++i) {
            if (src.client(i) == pickup_) {
                pos_p = i;
            }
            if (src.client(i) == delivery_) {
                pos_d = i;
            }
        }
        assert(pos_p >= 0 && pos_d >= 0);
...
bool SwapPair::find_best_move(Solution const& sol, CostEvaluator const& eval,
                              ProblemData const& data) {
    best_delta_ = 0;
    route_a_ = -1;
    ...
    for (int ra_req = 0; ra_req < num_req; ++ra_req) {
        ...
        auto try_swap_with = [&](int rb_req) {
            ...
            int64_t new_cost_a = find_best_pair_insertion(
                reduced_a, data, eval, route_a.vehicle_type(), pb, db, bp_b, bd_b);

            // Insert pair A into reduced route B at best positions.
            int bp_a, bd_a;
            int64_t new_cost_b = find_best_pair_insertion(
                reduced_b, data, eval, route_b.vehicle_type(), pa, da, bp_a, bd_a);
...
void SwapPair::apply(Solution& sol) const {
    ...
    for (int i = 0; i < route_a.size(); ++i) {
        if (route_a.client(i) == pickup_a_) {
            pos_pa = i;
        }
        if (route_a.client(i) == delivery_a_) {
            pos_da = i;
        }
    }
    for (int i = 0; i < route_b.size(); ++i) {
        if (route_b.client(i) == pickup_b_) {
            pos_pb = i;
        }
        if (route_b.client(i) == delivery_b_) {
            pos_db = i;
        }
    }
