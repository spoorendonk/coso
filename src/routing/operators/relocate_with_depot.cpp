...
int RelocateWithDepot::find_empty_route(Solution const& sol,
...
int RelocateWithDepot::count_trips(Solution const& sol,
                                    ProblemData const& data,
                                    int vehicle_type)
{
    int count = 0;
                                        ...
                                        ProblemData const& data)
{
    ...
    for (int r = 0; r < sol.num_routes(); ++r) {
        ...
        for (int pos = 1; pos < route.size(); ++pos) {
            ...
            int64_t new_cost = eval.route_cost(first_half)
                             + eval.route_cost(second_half);
    ...
    for (int ra = 0; ra < sol.num_routes(); ++ra) {
        ...
        for (int rb = ra + 1; rb < sol.num_routes(); ++rb) {
            ...
            int64_t old_cost = eval.route_cost(route_a)
