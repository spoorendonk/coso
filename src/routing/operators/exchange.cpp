...
inline int eval_remove_pair_dist(Route const& route,
                                 ProblemData const& data, int pos)
{
    ...
    return data.dist(profile, prev, next)
         - data.dist(profile, prev, u)
         - data.dist(profile, u, v)
         - data.dist(profile, v, next);
}

/// Load excess after removing two consecutive clients at pos, pos+1.
inline int eval_remove_pair_load(Route const& route,
                                 ProblemData const& data, int pos)
{
...
inline int64_t eval_remove_pair_cost(Route const& route,
                                     CostEvaluator const& eval,
                                     ProblemData const& data, int pos)
{
    auto const& vt = data.vehicle_type(route.vehicle_type());
    int64_t delta = 0;
    delta += static_cast<int64_t>(eval_remove_pair_dist(route, data, pos))
    ...
    delta += static_cast<int64_t>(new_excess - route.load_excess())
             * eval.load_penalty();
    return delta;
}

/// Distance delta for inserting clients c1, c2 consecutively at pos.
inline int eval_insert_pair_dist(Route const& route,
                                 ...
                                 int pos, int c1, int c2)
{
    ...
    int u = nd + c1;
    int v = nd + c2;
    return data.dist(profile, prev, u)
         + data.dist(profile, u, v)
         + data.dist(profile, v, next)
...
inline int eval_insert_pair_load(Route const& route,
                                 ProblemData const& data,
                                 int pos, int c1, int c2)
{
...
inline int64_t eval_insert_pair_cost(Route const& route,
                                     CostEvaluator const& eval,
                                     ProblemData const& data,
                                     int pos, int c1, int c2)
{
...
inline int eval_replace_dist(Route const& route, ProblemData const& data,
                             int pos, int new_client)
{
    int profile = data.vehicle_type(route.vehicle_type()).profile;
    int nd = data.num_depots();
...
inline int eval_replace_load(Route const& route, ProblemData const& data,
                             int pos, int new_client)
{
    auto const& left  = route.load_prefix(pos - 1);
...
inline int64_t eval_replace_cost(Route const& route,
                                 CostEvaluator const& eval,
                                 ProblemData const& data,
                                 int pos, int new_client)
{
...
/// to get exact cost.
inline int64_t eval_intra_relocate(Route const& route,
                                   CostEvaluator const& eval,
                                   ProblemData const& data,
...
bool Exchange10::find_best_move(Solution const& sol,
                                CostEvaluator const& eval,
                                ProblemData const& data)
{
    best_delta_ = 0;
            ...
            auto try_route = [&](int rb) {
                ...
                if (rb == ra) {
                    ...
                    for (int pb = 0; pb <= reduced; ++pb) {
                        ...
                        int64_t delta = eval_intra_relocate(
                            route_a, eval, data, pa, pb);
                    ...
                    for (int pb = 0; pb <= route_b.size(); ++pb) {
                        int64_t insert_delta = eval.eval_insert_cost(
                            route_b, pb, u);
...
bool Exchange11::find_best_move(Solution const& sol,
                                CostEvaluator const& eval,
                                ProblemData const& data)
{
    best_delta_ = 0;
...
bool Exchange20::find_best_move(Solution const& sol,
                                CostEvaluator const& eval,
                                ProblemData const& data)
{
    best_delta_ = 0;
    ...
    for (int ra = 0; ra < sol.num_routes(); ++ra) {
        ...
        for (int pa = 0; pa + 1 < route_a.size(); ++pa) {
            ...
            auto try_route = [&](int rb) {
                ...
                auto const& route_b = sol.route(rb);
                for (int pb = 0; pb <= route_b.size(); ++pb) {
                    int64_t insert_delta = eval_insert_pair_cost(
                        route_b, eval, data, pb, c1, c2);
                    int64_t delta = remove_delta + insert_delta;
                    if (delta < best_delta_) {
                        best_delta_ = delta;
                        from_route_ = ra;
...
bool SwapTails::find_best_move(Solution const& sol,
                               CostEvaluator const& eval,
                               ProblemData const& data)
{
    best_delta_ = 0;
