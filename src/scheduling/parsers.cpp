    ...
    for (int j = 0; j < num_jobs; ++j) {
        ...
        if (line.empty())
            throw std::runtime_error(
                "Taillard JSP parse error: expected job " + std::to_string(j));
        ...
        for (int o = 0; o < num_machines; ++o) {
            ...
            if (!(iss >> machine >> duration))
                throw std::runtime_error(
                    "Taillard JSP parse error: job " + std::to_string(j) +
    ...
    for (int a = 1; a <= num_jobs; ++a) {
        ...
        for (int r = 0; r < num_resources; ++r) {
            if (psp_jobs[a].resource_usage[r] > 0)
                builder.set_resource_usage(oidx, res_ids[r],
                                           psp_jobs[a].resource_usage[r]);
        }
    }
...
ScheduleData parse_fjsp(const std::string& content)
{
    ...
    for (int j = 0; j < num_jobs; ++j) {
        ...
        if (line.empty())
            throw std::runtime_error(
                "FJSP parse error: expected job " + std::to_string(j));
        ...
        if (!(iss >> num_ops) || num_ops <= 0)
            throw std::runtime_error(
                "FJSP parse error: invalid num_ops for job " + std::to_string(j));

        for (int o = 0; o < num_ops; ++o) {
            int num_eligible = 0;
            if (!(iss >> num_eligible) || num_eligible <= 0)
                throw std::runtime_error(
                    "FJSP parse error: job " + std::to_string(j) +
                    ", op " + std::to_string(o) + ": bad num_eligible");
