...
void parse_node_coord_section(std::istream& in, VrpInstance& inst)
{
    ...
    for (int k = 0; k < inst.dimension; ++k) {
        int id; double x, y;
        if (!(in >> id >> x >> y)) {
            throw std::runtime_error("NODE_COORD_SECTION: failed to read node " + std::to_string(k));
        }
        int idx = id - 1;
        if (idx < 0 || idx >= inst.dimension) {
            throw std::runtime_error("NODE_COORD_SECTION: node id " + std::to_string(id) + " out of range");
        }
        inst.coords[idx] = {x, y};
    }
    std::string rest;
...
void parse_demand_section(std::istream& in, VrpInstance& inst)
{
    ...
    for (int k = 0; k < inst.dimension; ++k) {
        ...
        int idx = id - 1;
        if (idx < 0 || idx >= inst.dimension) {
            throw std::runtime_error("DEMAND_SECTION: node id " + std::to_string(id) + " out of range");
        }
...
VrpInstance parse_vrp(const std::string& content)
{
    ...
    while (std::getline(in, line)) {
        ...
        if (upper == "NODE_COORD_SECTION") { parse_node_coord_section(in, inst); continue; }
        if (upper == "DEMAND_SECTION") { parse_demand_section(in, inst); continue; }
        if (upper == "DEPOT_SECTION") { parse_depot_section(in, inst); continue; }
        if (upper == "EDGE_WEIGHT_SECTION") { parse_edge_weight_section(in, inst); continue; }
        if (upper == "EOF") break;
    ...
    if (inst.edge_weight_type != EdgeWeightType::EXPLICIT && inst.coords.empty())
        throw std::runtime_error("VRP parse error: non-EXPLICIT instance requires NODE_COORD_SECTION");
...
VrpInstance read_solomon(std::istream& input)
{
    ...
    while (std::getline(input, line)) {
        ...
        if (upper == "CUSTOMER" || upper.find("CUST") != std::string::npos
            || upper.find("XCOORD") != std::string::npos
            || upper.find("COORD") != std::string::npos)
            continue;

        std::istringstream iss(trimmed);
        int id = 0; double x = 0, y = 0; int demand = 0, ready = 0, due = 0, service = 0;
...
VrpInstance read_lilim(std::istream& input)
{
    ...
    while (std::getline(input, line)) {
        ...
        if (!(iss >> node.id >> node.x >> node.y >> node.demand
                  >> node.earliest >> node.latest >> node.service
                  >> node.pickup_idx >> node.delivery_idx))
            break;
    ...
    for (int i = 0; i < inst.dimension; ++i) {
        ...
        if (n.id >= 0 && n.id <= max_id) id_to_index[n.id] = i;
