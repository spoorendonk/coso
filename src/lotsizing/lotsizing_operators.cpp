...
double compute_shift_delta(LotsizingData const& data, LotsizingSolution const& sol, int p, int from,
                           int to, double quantity) {
    double delta = 0.0;
...
ShiftProduction evaluate_shift(LotsizingSolution const& sol, int product, int from_period,
    ...
    shift.delta = compute_shift_delta(sol.data(), sol, product, from_period, to_period, quantity);
    return shift;
}

MergeSetups evaluate_merge(LotsizingSolution const& sol, int product, int source_period,
                           int target_period) {
    MergeSetups merge;
    ...
    merge.delta = compute_shift_delta(sol.data(), sol, product, source_period, target_period, qty);

    return merge;
}

SplitLot evaluate_split(LotsizingSolution const& sol, int product, int from_period, int to_period,
                        double quantity) {
    SplitLot split;
    ...
    split.delta = compute_shift_delta(sol.data(), sol, product, from_period, to_period, quantity);
    return split;
}
