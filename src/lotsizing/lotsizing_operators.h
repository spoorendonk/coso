...
[[nodiscard]] ShiftProduction evaluate_shift(
    LotsizingSolution const& sol,
    int product, int from_period, int to_period, double quantity);

/// Evaluate merging all production of product from source_period into
/// target_period.
[[nodiscard]] MergeSetups evaluate_merge(
    LotsizingSolution const& sol,
    int product, int source_period, int target_period);

/// Evaluate splitting `quantity` from a lot in from_period to to_period.
[[nodiscard]] SplitLot evaluate_split(
    LotsizingSolution const& sol,
    int product, int from_period, int to_period, double quantity);
...
[[nodiscard]] bool is_feasible(LotsizingSolution const& sol,
                               ShiftProduction const& shift);

/// Check if a merge is feasible (capacity in target_period).
[[nodiscard]] bool is_feasible(LotsizingSolution const& sol,
                               MergeSetups const& merge);

/// Check if a split is feasible.
[[nodiscard]] bool is_feasible(LotsizingSolution const& sol,
                               SplitLot const& split);
...
[[nodiscard]] std::vector<ShiftProduction> enumerate_shifts(
    LotsizingSolution const& sol);

/// Enumerate all feasible MergeSetups moves.
[[nodiscard]] std::vector<MergeSetups> enumerate_merges(
    LotsizingSolution const& sol);

/// Enumerate feasible SplitLot moves for overloaded periods.
[[nodiscard]] std::vector<SplitLot> enumerate_splits(
    LotsizingSolution const& sol);
