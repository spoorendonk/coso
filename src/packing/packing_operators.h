...
[[nodiscard]] std::vector<SwapItems> enumerate_swaps(PackingSolution const& sol);

/// Evaluate the cost delta for merging source_bin into target_bin.
[[nodiscard]] MergeBins evaluate_merge(PackingSolution const& sol, int source_bin, int target_bin);

/// Check if a merge is feasible (all source items fit in target bin
/// capacity-wise and conflict-wise).
[[nodiscard]] bool is_feasible(PackingSolution const& sol, MergeBins const& merge);
...
[[nodiscard]] std::vector<MergeBins> enumerate_merges(PackingSolution const& sol);

/// Evaluate a split: move items_to_move from source_bin to target_bin.
[[nodiscard]] SplitBin evaluate_split(PackingSolution const& sol, int source_bin, int target_bin,
                                      std::vector<int> items_to_move);

/// Check if a split is feasible (moved items fit in target bin).
[[nodiscard]] bool is_feasible(PackingSolution const& sol, SplitBin const& split);
...
[[nodiscard]] std::vector<SplitBin> enumerate_splits(PackingSolution const& sol);
