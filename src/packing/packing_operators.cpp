...
MergeBins evaluate_merge(PackingSolution const& sol, int source_bin, int target_bin) {
...
SplitBin evaluate_split(PackingSolution const& sol, int source_bin, int target_bin,
...
bool is_feasible(PackingSolution const& sol, SplitBin const& split) {
    ...
    for (int i = 0; i < static_cast<int>(split.items_to_move.size()); ++i) {
        for (int j = i + 1; j < static_cast<int>(split.items_to_move.size()); ++j) {
            if (sol.data().has_conflict(split.items_to_move[i], split.items_to_move[j])) {
                return false;
            }
        }
    }
...
std::vector<SplitBin> enumerate_splits(PackingSolution const& sol) {
    ...
    for (int b = 0; b < B; ++b) {
        ...
        if (target < 0) {
            continue;  // no empty bin available
        }
        ...
        std::sort(sorted_items.begin(), sorted_items.end(), [&](int a, int ab) {
            return sol.data().item_size(a, 0) > sol.data().item_size(ab, 0);
        });
