...
public:
    ...
    explicit AssignmentCostEvaluator(AssignmentData const& data)
        : data_(data), weights_()
    {
    }
    ...
    {
        return demand_cost(schedule)
             + hard_constraint_cost(schedule)
             + preference_cost(schedule)
    ...
    [[nodiscard]] bool is_feasible(
        std::vector<std::vector<int>> const& schedule) const
    {
        return hard_constraint_cost(schedule) == 0
            && unavailability_cost(schedule) == 0;
    }

    // -- Component costs -----------------------------------------------------

    /// Demand violation cost (under/over-staffing).
    [[nodiscard]] int demand_cost(
        std::vector<std::vector<int>> const& schedule) const
    {
        int cost = 0;
                ...
                for (int e = 0; e < ne; ++e) {
                    if (schedule[e][d] == s) {
                        if (dem.required_skill.empty()
                            || has_skill(e, dem.required_skill)) {
                            ++count;
                        }
                    }
                }
    ...
    [[nodiscard]] int hard_constraint_cost(
        std::vector<std::vector<int>> const& schedule) const
    {
        return consecutive_violation_cost(schedule)
             + rest_violation_cost(schedule)
             + forbidden_sequence_cost(schedule)
    ...
    [[nodiscard]] int rest_violation_cost(
        std::vector<std::vector<int>> const& schedule) const
    {
        int cost = 0;
        int const ne = data_.num_employees();
    ...
    [[nodiscard]] int forbidden_sequence_cost(
        std::vector<std::vector<int>> const& schedule) const
    {
        int cost = 0;
        int const ne = data_.num_employees();
    ...
    [[nodiscard]] int unavailability_cost(
        std::vector<std::vector<int>> const& schedule) const
    {
        int cost = 0;
        int const ne = data_.num_employees();
        ...
        std::vector<std::vector<int>> const& schedule) const
    {
        ...
        int cost = 0;
        int const ne = std::min(data_.num_employees(),
                                static_cast<int>(data_.published_schedule.size()));
        for (int e = 0; e < ne; ++e) {
            int const days = std::min(data_.horizon,
                                      static_cast<int>(data_.published_schedule[e].size()));
    ...
    [[nodiscard]] int delta_assign(
        std::vector<std::vector<int>> const& schedule,
        int employee, int day, int shift_type) const
    {
    ...
    [[nodiscard]] int delta_unassign(
        std::vector<std::vector<int>> const& schedule,
        int employee, int day) const
    {
        auto copy = schedule;
        int before = evaluate(copy);
        copy[employee][day] = -1;
    ...
    [[nodiscard]] int delta_swap(
        std::vector<std::vector<int>> const& schedule,
        int emp1, int emp2, int day) const
    {
        auto copy = schedule;
        int before = evaluate(copy);
...
private:
    [[nodiscard]] bool has_skill(int employee,
                                 std::string const& skill) const
    {
