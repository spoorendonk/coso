...
int assignment_total_understaffing(AssignmentData const& data,
...
int assignment_total_hard_violations(AssignmentData const& data,
...
int64_t assignment_overconstrained_penalty(AssignmentData const& data,
                                           AssignmentCostEvaluator const& eval,
                                           std::vector<std::vector<int>> const& schedule,
    ...
    penalty += static_cast<int64_t>(assignment_total_hard_violations(data, eval, schedule)) *
               config.constraint_violation_penalty;

    return penalty;
}

int64_t assignment_overconstrained_cost(AssignmentData const& data,
                                        AssignmentCostEvaluator const& eval,
                                        std::vector<std::vector<int>> const& schedule,
                                        AssignmentOverconstrainedConfig const& config) {
    ...
    return base + assignment_overconstrained_penalty(data, eval, schedule, config);
}

bool assignment_overconstrained_feasible(AssignmentCostEvaluator const& eval,
                                         std::vector<std::vector<int>> const& schedule) {
