...
/// Move descriptor for delta evaluation in the rostering engine.
///
...
/// Abstract base class for rostering constraints.
///
...
public:
    ...
    [[nodiscard]] virtual int evaluate(RosteringData const& data,
    ...
    [[nodiscard]] virtual int evaluate_delta(RosteringData const& data,
...
private:
    ...
    [[nodiscard]] int count_assigned(RosteringData const& data,
...
private:
    ...
    [[nodiscard]] int employee_cost(RosteringData const& data, std::vector<int> const& row,
                                    int horizon) const;
};
...
public:
    ...
    [[nodiscard]] std::vector<std::pair<std::string, int>> breakdown(
        RosteringData const& data, std::vector<std::vector<int>> const& schedule) const {
