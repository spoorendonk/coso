...
/// Constraint propagation filter for the rostering engine.
///
...
/// prune a list of RosteringMove candidates.
class CPFilter {
public:
    explicit CPFilter(RosteringData const& data)
        : data_(data),
    ...
    int filter_moves(std::vector<RosteringMove>& moves) const {
        auto original_size = static_cast<int>(moves.size());
        moves.erase(std::remove_if(moves.begin(), moves.end(),
                                   [this](RosteringMove const& m) {
