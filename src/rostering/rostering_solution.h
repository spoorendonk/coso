...
/// Solution representation for the rostering engine.
///
...
class RosteringSolution {
public:
    /// Construct an empty solution (all cells = -1).
    explicit RosteringSolution(RosteringData const& data,
                                RosteringCostEvaluator const& evaluator)
        : data_(data),
          evaluator_(evaluator),
          schedule_(data.num_employees(), std::vector<int>(data.horizon, -1)),
          cost_(evaluator.evaluate(schedule_)) {}

    // -- Accessors -----------------------------------------------------------

    /// Number of employees.
    [[nodiscard]] int num_employees() const noexcept { return data_.num_employees(); }
    ...
    [[nodiscard]] int get(int employee, int day) const { return schedule_[employee][day]; }
    ...
    [[nodiscard]] bool is_feasible() const { return evaluator_.is_feasible(schedule_); }
    ...
    void recompute_cost() { cost_ = evaluator_.evaluate(schedule_); }

    // -- Evaluation helpers --------------------------------------------------

    /// Demand cost component.
    [[nodiscard]] int demand_cost() const { return evaluator_.demand_cost(schedule_); }
    ...
    [[nodiscard]] int preference_cost() const { return evaluator_.preference_cost(schedule_); }

private:
    RosteringData const& data_;
    RosteringCostEvaluator const& evaluator_;
